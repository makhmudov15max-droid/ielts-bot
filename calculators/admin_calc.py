...
def calculate_admin_salary(data):
    ...
    individual_kpi = safe_division(
        data["actual_sales"],
        data["individual_plan"]
    )

    conversion_kpi = safe_division(
        data["actual_conversion"],
        data["conversion_plan"]
    )

    active_kpi = safe_division(
        data["actual_active"],
        data["active_plan"]
    )
    ...
    russian_bonus = 500000 if data["knows_russian"] else 0
    ielts_bonus = 1000000 if data["has_ielts"] else 0
