...
from utils.tasks_db import save_tasks, load_tasks, insert_task
...
async def show_today_all_tasks(message: types.Message, state: FSMContext):
    ...
    response_text = f"📅 <b>Bugungi ({today}) vazifalar:</b>\n\n"
    for idx, task in enumerate(all_tasks, 1):
        ...
        if task.get("task_type") == "Kunlik (Bir martalik)":
            response_text += (
                ...
                f"   📝 Izoh: {task.get('task_description', 'Mavjud emas')}\n"
            ...
            response_text += (
                ...
                f"   👤 Masʻul: {task['assigned_to_name']}\n"
                f"   ⏰ Vaqti: {', '.join(task.get('task_times', []))}\n"
                f"   📅 Kunlar: {task.get('task_days', '-')}\n"
                f"   📸 Isbot: {task['proof_type']}\n"
                f"   {status_text}\n\n"
            )
